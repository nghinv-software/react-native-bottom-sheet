...
function BottomSheetViewComponent(props?: BottomSheetViewProps, ref: React.Ref) {
  ...
  const [checkPosition, setCheckPosition] = useState(props.checkPosition);
                            ...
                            options.map((option, idx) => (
                              <React.Fragment key={String(idx)}>
                                ...
                                <Button
                                  {...optionProps}
                                  {...option}
                                  checked={!isNullOrUndefined(checkPosition) ? checkPosition === idx : option.checked}
                                  onPress={() => {
                                    option.autoDismiss !== false && onDismissModal();
                                    if (!isNullOrUndefined(checkPosition)) {
                                      setCheckPosition(idx);
                                    }
                                    if (nativeModal) {
                                      setTimeout(() => {
